...
import { Plus, Edit2, Trash2, Loader2, AlertCircle, X, Building2, CheckCircle } from "lucide-react";
...
export default function DepartmentsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(null);
      if (editingDept) {
        await updateDepartmentApi(editingDept.id, { name, description });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDepartmentApi(deletingId);
      ...
      if (err instanceof Error) setError(err.message);
      else setError("Cannot delete department with assigned employees.");
      setDeletingId(null);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Organize hotel operations into functional service units.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center">
          ...
          <h3 className="font-bold text-slate-800 text-base">No Departments</h3>
          <p className="text-xs text-slate-500 mt-1">Get started by creating your first hotel department.</p>
        </div>
      ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Responsibilities and notes..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-amber-500"
                />
              </div>
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Are you sure? Departments with assigned employees cannot be removed until staff are reassigned.
            </p>
